...
void testQuantity() {//add lots of elements
	...
	for (int i = 10; i >= 1; i--) { //delete existing and inexistent elements
		for (int j = -30000; j < 30000; j = j + i) {
			s.remove(j);
		}
	}
...
void testPrevious()
{
	cout << "Test previous iterator" << endl;
	Set s;
	...
	// If first element previous makes the iterator invalid
	it.first();
	it.previous();
	assert(it.valid() == false);

	// If iterator is invalid it throws an exception
	try {
		it.previous();
		assert(false);
	}
